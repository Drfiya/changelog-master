...
export function ChatPanel({ versions, isOpen, onClose }: ChatPanelProps) {
  ...
  return (
    <>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-5xl h-[85vh] rounded-xl shadow-2xl z-50 flex overflow-hidden" style={{ background: 'var(--bg-primary)' }}>
        {/* Conversation History Sidebar */}
        {showHistory && (
          <div className="w-64 border-r flex flex-col" style={{ borderColor: 'var(--border-light)', background: 'var(--bg-secondary)' }}>
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {conversations.length === 0 ? (
                <p className="text-sm text-[var(--text-tertiary)] text-center py-4">No conversations yet</p>
              ) : (
                conversations.map((conv) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1 hover:bg-[var(--accent-red)]/20 rounded transition-all"
                    >
                      <Trash2 className="w-3 h-3 text-[var(--accent-red)]" />
                    </button>
                  </div>
                ))
              )}
        ...
        <div className="w-56 border-r flex flex-col" style={{ borderColor: 'var(--border-light)' }}>
          <div className="p-3 border-b" style={{ borderColor: 'var(--border-light)' }}>
            <h3 className="font-semibold text-[var(--text-primary)] text-sm mb-1">Select Releases</h3>
            ...
            <div className="flex gap-1">
              <button
                onClick={selectAll}
                className="flex-1 text-xs px-2 py-1 bg-[var(--accent-navy)]/20 text-[var(--accent-navy)] rounded-lg hover:bg-[var(--accent-navy)]/30 transition-colors"
              >
                All
              </button>
              <button
              ...
              <button
                ...
                className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-lg mb-0.5 transition-colors text-left ${
                  selectedVersions.has(v.version)
                    ? 'bg-[var(--accent-navy)]/20 text-[var(--accent-navy)]'
              ...
              >
                <div
                  className={`w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 ${
                    selectedVersions.has(v.version)
                      ? 'bg-[var(--accent-navy)] border-[var(--accent-navy)]'
          ...
          <div className="p-2 border-t" style={{ borderColor: 'var(--border-light)', background: 'var(--bg-secondary)' }}>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex items-center justify-between p-3 border-b" style={{ borderColor: 'var(--border-light)' }}>
            <div className="flex items-center gap-2">
              <button
                ...
                className={`icon-btn ${
                  showHistory ? 'bg-[var(--accent-navy)]/20 text-[var(--accent-navy)]' : ''
                }`}
                title="Chat history"
              >
                ...
                <div className="grid grid-cols-1 gap-2 max-w-sm">
                  {[
                    ...
                    <button
                      ...
                      className="text-left text-sm px-4 py-2 rounded-xl hover:bg-[var(--bg-tertiary)] transition-colors"
              ...
              messages.map((message) => (
                ...
                >
                  <div
                    ...
                    style={message.role === 'assistant' ? { background: 'var(--bg-secondary)', color: 'var(--text-primary)' } : undefined}
                  >
                    {message.role === 'user' ? (
          ...
          <div className="p-4 border-t" style={{ borderColor: 'var(--border-light)' }}>
            {selectedVersions.size === 0 && (
              <div className="mb-2 text-xs text-[var(--accent-blue)] flex items-center gap-1">
            ...
            <div className="flex gap-2">
              <textarea
                ...
                rows={1}
                className="flex-1 resize-none border-0 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[var(--accent-blue)] focus:outline-none"
